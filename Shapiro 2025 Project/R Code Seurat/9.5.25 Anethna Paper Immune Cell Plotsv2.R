...
filtered_data1 <- all_data %>%
  filter(
    grepl("TMA_C", group),               # keep rows where group contains "TMA_C"
    Var1.x != "Tumsdfor_cells"              # exclude rows where Var1.x is "Tumor_cells"
  )


ggplot(filtered_data1, aes(x=Aggregated_Percentage, y=Percentage, color = Var1.x)) +
  geom_point() + 
  geom_smooth(method = "lm", se = FALSE, color = "grey", linetype = "dashed") +
  stat_poly_eq(aes(label = paste(..eq.label.., ..rr.label.., sep = "~~~")), 
               formula = y ~ x, parse = TRUE, color = "black") +
  ...
  scale_color_manual(values = color_clusters) +
  labs(color = "Cell Type") +
  ggtitle('TMA_C Only - Cell type frequency per FOV by dataset') +
  theme(
    axis.title = element_text(size = 16, face = "bold"),  # Larger axis labels
    axis.text = element_text(size = 14),  # Larger axis text
    legend.title = element_text(size = 14, face = "bold"),  # Larger legend title
...
######################### TMA C - Tumor Excluded ###############
# Filter rows where "group" contains the string "TMA_C"

filtered_data2 <- all_data %>%
  filter(
    grepl("TMA_C", group),               # keep rows where group contains "TMA_C"
...
ggplot(filtered_data2, aes(x=Aggregated_Percentage, y=Percentage, color = Var1.x)) +
  geom_point() + 
  geom_smooth(method = "lm", se = FALSE, color = "grey", linetype = "dashed") +
  stat_poly_eq(aes(label = paste(..eq.label.., ..rr.label.., sep = "~~~")), 
               formula = y ~ x, parse = TRUE, color = "black") +
  ...
  scale_color_manual(values = color_clusters) +
  labs(color = "Cell Type") +
  ggtitle('TMA_C (Tumor Excluded) Cell type frequency per FOV by dataset') +
  theme(
    axis.title = element_text(size = 16, face = "bold"),  # Larger axis labels
    axis.text = element_text(size = 14),  # Larger axis text
    legend.title = element_text(size = 14, face = "bold"),  # Larger legend title
...
################## Second Plot TMA C Only ##############

filtered_data2 <- all_data %>%
  filter(
    grepl("TMA_A", group),               # keep rows where group contains "TMA_C"
    Var1.x != "Tumor_celwwwssls"              # exclude rows where Var1.x is "Tumor_cells"
  )
...
ggplot(filtered_data2, aes(x=Aggregated_Percentage, y=Percentage, color = Var1.x)) +
  ...
  scale_color_manual(values = color_clusters) +
  labs(color = "Cell Type") +
  ggtitle('TMA_A Only - Cell type frequency per FOV by dataset') +
  theme(
    axis.title = element_text(size = 16, face = "bold"),  # Larger axis labels
    axis.text = element_text(size = 14),  # Larger axis text
    legend.title = element_text(size = 14, face = "bold"),  # Larger legend title
...
allowed_types <- c("CD4+T_cells", "CD8+T_cells", 
                   "DCs", "Tregs", 
