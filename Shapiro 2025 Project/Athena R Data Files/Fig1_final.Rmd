...
# TEST CosMx protein recluster (grouping transferred clusters with parent population)
```{r}
...
mac <- colnames(tumor_protein)[tumor_protein$Protein_snn_res.0.01 %in% c(3, 28, 21, 12, 11)]

CosMx_Protein_log_norm$merged_annot_cluster[colnames(CosMx_Protein_log_norm) %in% c(tregs)] <- 'Tregs'
CosMx_Protein_log_norm$merged_annot_cluster[colnames(CosMx_Protein_log_norm) %in% c(endo)] <- 'Endothelial_cells'
CosMx_Protein_log_norm$merged_annot_cluster[colnames(CosMx_Protein_log_norm) %in% c(nk)] <- 'NK_cells'
CosMx_Protein_log_norm$merged_annot_cluster[colnames(CosMx_Protein_log_norm) %in% c(tcells)] <- 'Tcells'
CosMx_Protein_log_norm$merged_annot_cluster[colnames(CosMx_Protein_log_norm) %in% c(neut)] <- 'Neutrophils'
CosMx_Protein_log_norm$merged_annot_cluster[colnames(CosMx_Protein_log_norm) %in% c(mac)] <- 'Macrophages'
...
ggsave('TESTING_CosMx_Protein_DE_genes.pdf', width = 6, height = 6)
```
...
# Table of TMA core by patient ID
```{r}
meta <- CosMX_RNA_merged[[c('tma_fov_id', 'PatientID')]]
meta <- unique(meta)

write.csv(meta, file = 'CosMx_PatientID_TMA_CORE.csv')
```
...
features <- c('MS4A1', 'CD79A', 'IGHG2', 'HLA-DRB1', 'HLA-DRA', 'LAMP3', 'CLEC9A', 'CD1C', 
              'PECAM1', 'CD34', 'VIM', 'DCN', 'COL1A2', 'CD74','HLA-C', 'VIM', 'ACTA2', 'VEGFA', 
              'COL1A1', 'C1QC', 'C1QA', 'CD68', 'CD14', 'FCGR3A', 'S100A8', 'S100A9', 'VCAN', 'FCN1', 'CD38', 'JCHAIN', 
...
DotPlot_aeg(CosMX_RNA_merged, group.by = "annot_cluster2", features = unique(features), col.min=0, cols = c("lightgray", "red")) +
  theme(axis.text.x = element_text(angle = 90, vjust = 1, hjust=1)) + theme(legend_title = 'Avg Expression') 
ggsave('cosmx_rna_degenes.pdf', width = 9, height = 5)
```
...
DotPlot(hnc2, group.by = 'global.cluster4', features = c('APOBEC3A', 'TIMP1', 'CXCL10', 'CXCL8', 'LILRB2', 'FCGR3A', 'KLF2', 'LST1', 'CLEC4E', 'IL1R2', 'THBS1', 
                                                         ...
                                                         'KIT', 'APOC1', 'MMP9', 'PLTP', 'FABP5', 'HSPB1', 'SPP1', 'EREG', 'CCL4', 'IL1B', 'APOE', 'CXCL9', 'C1QC', 'C1QA',
...
ggsave('Cillo_DE_dotplot.pdf', width = 7, height = 12)
```
